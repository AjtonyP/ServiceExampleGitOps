...
resource "null_resource" "generate_kubeconfig" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      microk8s config > ${var.kubeconfig_path}
...
resource "null_resource" "bootstrap_flux" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      if microk8s kubectl get crd gitrepositories.source.toolkit.fluxcd.io 2>/dev/null | grep -q gitrepositories; then
      ...
      microk8s kubectl wait --for=condition=ready pod -l app=source-controller -n flux-system --timeout=300s
      microk8s kubectl wait --for=condition=ready pod -l app=kustomize-controller -n flux-system --timeout=300s
      microk8s kubectl wait --for=condition=ready pod -l app=helm-controller -n flux-system --timeout=300s
      
      # Wait for CRDs to be established
      echo "Waiting for Flux CRDs to be ready..."
      sleep 10
      microk8s kubectl wait --for condition=established --timeout=60s crd/gitrepositories.source.toolkit.fluxcd.io
      microk8s kubectl wait --for condition=established --timeout=60s crd/helmrepositories.source.toolkit.fluxcd.io
      microk8s kubectl wait --for condition=established --timeout=60s crd/helmreleases.helm.toolkit.fluxcd.io
      microk8s kubectl wait --for condition=established --timeout=60s crd/kustomizations.kustomize.toolkit.fluxcd.io
...
resource "null_resource" "apply_gitops_config" {
  ...
  provisioner "local-exec" {
    command = <<-EOT
      ...
      microk8s kubectl get crd gitrepositories.source.toolkit.fluxcd.io
      microk8s kubectl get crd helmrepositories.source.toolkit.fluxcd.io
      
      # Apply GitRepository and HelmRepository sources
      echo "Applying sources..."
      microk8s kubectl apply -f ${path.module}/../flux/sources/
      ...
      microk8s kubectl apply -k ${path.module}/../flux/infrastructure/
      ...
      microk8s kubectl apply -k ${path.module}/../flux/apps/
